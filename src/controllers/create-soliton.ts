  ...
  type ApiResponse = SuccessResponse | ErrorResponse;
    ...
    if (response.data.status === 'error') {
      if (response.data.error === 'no user with this phone number') {
        const response = await axios.post<ApiResponse>('http://soliton.net.ua/water/api/user/add/index.php', {
          phone_number: phone,
          first_name,
          last_name,
          date_birth,
      ...
      apiUser
        ? await prisma.apiusers.update({
          ...
          data: {
            ...
            cardId: response.data.user.card[0]?.ID ? Number(response.data.user.card[0].ID) : null,
          },
        })
        : await prisma.apiusers.create({ // <-- відкриваємо дужку об'єкта аргументів
          data: {
              user_id: parseInt(response.data.user.uid),
              chat_id: chat_id,
              name: response.data.user.name,
    ...
    if (newUser) {
      ...
      try {
        ...
        const userCard = prisma.users.update({where: {chat_id}, 
          data: { card_id: Number(cardId) }},
      {
            solitonCardId
        });
    } catch (error) {
